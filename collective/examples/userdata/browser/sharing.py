...
def merge_search_results(results, key):
    """Merge member search results.role_settings
...
class OverrideSharingView(BrowserView):
    ...
    def roles(self):
        ...
        pairs.sort(key=lambda x: normalizeString(translate(x["title"],
    ...
    def role_settings(self):
        ...
        for setting in current_settings:
            if setting['type'] == 'user':
                user = api.user.get(username=setting['id'])
                company = user.getProperty('company')
                setting['title'] = '%s / %s' % (setting['title'], company)
